...
`inputrc`文件的位置由`shell`的环境变量`INPUTRC`控制，如果该变量没有设置，缺省的inputrc文件的路径是~/.inputrc。

如果该文件~/.inputrc不存在，就会使用系统级(对所有用户生效)的`inputrc`文件`/etc/inputrc`。如果某个用户需要修改系统默认的inputrc配置，可以改动`~/.inputrc`，这样会覆盖系统的默认配置。
