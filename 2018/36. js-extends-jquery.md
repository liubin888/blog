jquery 截止到当前已经3.3.1版本了，如今随着各种浏览器的盛行，前端的框架层出不穷，jquery独步天下，老夫写代码只用jquery，拿起代码就是干的辉煌时代已经过去了。
...
就在微软收购github 的52天，github改变也已经放弃了jquery，奇替代方案使用了原生的js：   
...
本文粗燥的实现jquery 的 ready、each、bind、$.fn.extend、$.extend   
...
;(function (win) {
  ...
  _$.prototype = {
    ...
    Init: function (selector, context) {
      this.elements = [];
      /**
       * 传入的类型是function 就执行ready事件，如果是document 就将document对象插入到this.elements
  ...
  /**
   * 让两个作用域不一样的对象共享一个方法，让他们的原型指向一致，即Init.prototype = _$.prototype
   * 那么原型一致之后 就可以共享this.elements 属性了。
   */
...
ready: function (callback) {
  ...
  if (isDocument(this.elements[0])) {   
    if (document.addEventListener) { //判断火狐、谷歌
       ...
       * 
       * arguments.callee 博客里面有一篇文章 [js-递归] 里面专门讲到了，这里不再解释了
       */
    ...
    } else if (document.lastChild == document.body) { //body已经加载完了，就直接回调了
      callback()
    }
  }
},
...
```
这俩看上去一模一样啊，没啥区别，注释里面已经说了，this指向不同。咱们来看个例子：
```html
<!DOCTYPE html>
<html>
<head>
...
; (function (win) {
    ...
    ready: function (callback) {
      ...
      if (isDocument(this.elements[0])) {
        ...
        if (document.addEventListener) { //判断火狐、谷歌
           ...
           * 这也就是$(document).ready() 比 window.onload 执行早的原因
           * 
           * arguments.callee 博客里面有一篇文章 js-递归里面专门讲到了，这里不再解释了
           */
          document.addEventListener('DOMContentLoaded', function () {
            document.removeEventListener('DOMContentLoaded', arguments.callee, false)
            callback()
  ...
  /**
   * 让两个作用于不一样的对象共享一个方法，让他们的原型指向一直，即Init.prototype = _$.prototype
